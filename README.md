...
- I prompt vengono copiati sempre in `.codex/.environment/prompts` senza sovrascrivere file già presenti: la sorgente è la prima cartella `prompts` trovata in ordine `--config` (se è una directory), `CODEX_CONFIG_DIR`/`~/.codex`/`~/.config/codex`/`$WORK_DIR/codex`/`$WORK_DIR/.codex` (ovvero il `CODEX_HOME` host effettivo).
