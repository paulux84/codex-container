...
- Il contenuto della prima cartella `prompts` disponibile (in ordine: directory passata a `--config` se è una directory, poi `CODEX_CONFIG_DIR`/`~/.codex`/`~/.config/codex`/`$WORK_DIR/codex`/`$WORK_DIR/.codex`) deve essere copiato in `.codex/.environment/prompts` senza sovrascrivere file esistenti, così i prompt definiti sull'host diventano disponibili nel container.
